...
const SESSION_DIR = process.env.SESSION_DIR || path.resolve(process.cwd(), ".wwebjs_auth");
...
function main(): void {
  ...
  console.log(
    `[engine] ENGINE_TOKEN: ${ENGINE_TOKEN ? "set" : "not set (dev mode)"}`
  );
